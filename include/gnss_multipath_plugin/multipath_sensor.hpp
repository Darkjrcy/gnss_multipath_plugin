...
/**
  ...
  \endcode
*/
class GazeboRosMultipathSensor : public gazebo::SensorPlugin
{
public:
  /// \brief Constructor
  GazeboRosMultipathSensor();

  /// \brief Destructor
  virtual ~GazeboRosMultipathSensor();

  // Documentation Inherited
  void Load(gazebo::sensors::SensorPtr _parent, sdf::ElementPtr _sdf);

private:
  std::unique_ptr<GazeboRosMultipathSensorPrivate> impl_;
};
