...
class TargetTraj(Node):
    ...
    def __init__(self, target_name=None, traj_f=None, *traj_args) -> None:
        ...
        #self.state_msg.reference_frame = "hong_kong" # default frame
    ...
    def traj_callback(self):
        ...
        self.state_msg.pose.position.x = _pose[0] + 0.01*np.random.normal()
        self.state_msg.pose.position.y = _pose[1] + 0.01*np.random.normal()
        self.state_msg.pose.position.z = _pose[2] + 0.01*np.random.normal()

        _q = quaternion_from_euler(_pose[3], _pose[4], _pose[5])
        self.state_msg.pose.orientation.x = _q[0] + 0.01*np.random.normal()
        self.state_msg.pose.orientation.y = _q[1] + 0.01*np.random.normal()
        self.state_msg.pose.orientation.z = _q[2] + 0.01*np.random.normal()
        self.state_msg.pose.orientation.w = _q[3] + 0.01*np.random.normal()
