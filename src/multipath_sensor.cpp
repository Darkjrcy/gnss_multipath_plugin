...
public:
  ...
  bool disable_noise_;
  // COnvert geodetic coordinates message
  geodetic_converter::GeodeticConverter g_geodetic_converter;
  // Total number of satellites
  int num_sat_ = 50;
  // Maximum number of satellites used:
  int max_num_sat_;
  int base_max_num_sat_;
  // TlE parameters to store the satellite's orbital parameters
  std::vector<std::string> tle_lines_;
  ...
  // Define the initial latitude, longitude and altitude of the origin
  double origin_lat_, origin_lon_, origin_alt_;
  // Delay time of the GPS message:
  double dt_;
  ...
  // Subscribe to know when the dropout occurs:
  ...
  // Callback function for the dropout message:
  void DropOutCallback(const std_msgs::msg::Bool::SharedPtr msg) {
    dropout_flag_ = msg->data;
    RCLCPP_INFO(ros_node_->get_logger(), "drop_out_flag := %s", dropout_flag_ ? "true" : "false");
  }
...
void GazeboRosMultipathSensor::Load(gazebo::sensors::SensorPtr _sensor, sdf::ElementPtr _sdf)
{
  ...
  // CHange teh characteristics using the sdf tags:
  ...
  // Define min intesity for the SensorRay to work:
  ...
  // Define the value of the delay:
  if (!_sdf->HasElement("delay_time"))
  {
    RCLCPP_DEBUG(impl_->ros_node_->get_logger(),  "The GNSS has not delay");
    impl_->dt_ = 0.0;
  } 
  else
  {
    std::string dt = _sdf->Get<std::string>("delay_time");
    std::istringstream ss(dt);
    double delay_time;
    ss >> delay_time;
    impl_->dt_ = delay_time;
  }

  // Define the maximum number of satellites:
  ...
  {
    impl_->max_num_sat_ = impl_->num_sat_;
  }
  ...
  // Create a subscriber to know when the dropout would occur:
  impl_->dropout_sub_ = impl_->ros_node_->create_subscription<std_msgs::msg::Bool>("drop_out_flag",rclcpp::QoS(10),std::bind(&GazeboRosMultipathSensorPrivate::DropOutCallback, impl_.get(), std::placeholders::_1));


  // Subscribe to the laserScan sensor output from gazebo:
...
void GazeboRosMultipathSensorPrivate::PublishLaserScan(ConstLaserScanStampedPtr & _msg)
{
  ...
  // Change the maximum number of satellites when the dropout flag is false:
  if (dropout_flag_)
  {
    max_num_sat_ = 3;
  } 
  ...
  {
    max_num_sat_ = base_max_num_sat_;
  }
      ...
      if (ray_ranges_[2*i] < _msg->scan().range_max()) 
      {
        ...
        if (mir_ray_range_ < _msg->scan().range_max())
        {
          ...
          if (range_offset < 50)
          {
            ...
            visible_sat_range_meas.push_back(sat_true_range_[i] + range_offset +  noise_ + c0_*dt_);
  ...
  if ((vis_num_sat_ - num_sat_blocked_) > 2) //Normally 4
  {
...
// Function to Change the ray angles to the satellite position and find hteir distance and orientation
...
void GazeboRosMultipathSensorPrivate::ParseSatelliteTLE()
{
  ...
  RCLCPP_INFO(ros_node_->get_logger(), "Maximum number of satellites used for the GPS estimation:%d", max_num_sat_);
